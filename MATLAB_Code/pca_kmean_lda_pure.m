...
for i=1:c  
    trnd{i} = trnd{i}(:,1:NpC);    
    TSum = TSum + sum(trnd{i},2); % total sum of training samples
end 
...
for i=1:c  
    trnd{i}  = trnd{i}(live_pixels,1:NpC); % excluding ever-off pixels to reduce dimentionality
    for j=1:NpC
        S_t = S_t + (trnd{i}(:,j)-TMean)*(trnd{i}(:,j)-TMean)'/NT; 
    end 
    clc % counter display
...
sptrnd = cell(Spc,1); % split data ensemble containing the entire clusters from k-means
for i=1:c 
    [INDX,M] = kmeans((pcatrnd{i}(:,1:NpC))',SpSpec(i));,...
%                       'start',pcatrnd{i}(:,SpSpec(i):floor(NpC/SpSpec(i)):NpC)');
    for j=0:SpSpec(i)-1
        sptrnd{sum(SpSpec(1:i))-j}   = pcatrnd{i}(:,INDX==j+1);         % new split classes
        SCMean(:,sum(SpSpec(1:i))-j) = M(j+1,:)';                       % split class mean
        [~,SNpC(sum(SpSpec(1:i))-j)]= size(sptrnd{sum(SpSpec(1:i))-j}); % No. samples per split class
    end
    clc % counter display
...
for m=1:c                    % test ensemble is PCA and LDA transformed 
    newtstd{m} = TLDA'*TPCA'*tstd{m}(live_pixels,1:TstNpC); 
 end
for i=1:Spc  
    Cov(:,:,i) = zeros(Spc-1,Spc-1);
    ldatrnd{i} = TLDA'*sptrnd{i}; % clustered training ensemble is LDA transformed  
    Mu(:,i) = TLDA'*SCMean(:,i);              % new class means (Spc-1)-by-Spc
    for j=1:SNpC(i)             % new class covarince matrix (Spc-1)by(Spc-1)by(Spc)
        Cov(:,:,i) = Cov(:,:,i) + (ldatrnd{i}(:,j)-Mu(:,i))*(ldatrnd{i}(:,j)-Mu(:,i))'/SNpC(i);
    end
...
pure_index=zeros(TstNpC,10);     % will be one whenever a misprediction occures
for i=1:c
    syndrm = 0;      % error event syndrome
    for j=1:TstNpC
        for k=1:Spc  % squared Mahalanobis distance for each tests sample within each class
            mhd2(k) = (newtstd{i}(:,j)-Mu(:,k))'*((Cov(:,:,k)\(newtstd{i}(:,j)-Mu(:,k))))...
                       /2+dist_term(k);          
        end
    ...
    Pe_pure(i) = syndrm/TstNpC; % misclassification rate for each class 
    clc % counter display
    display(['Error Computation: ',num2str(floor(100*i/c)),'%']);
end
Pe_pure_total = mean(Pe_pure)  % Average misclassification error
save('pure_results','Pe_pure','Pe_pure_total','pure_index')  